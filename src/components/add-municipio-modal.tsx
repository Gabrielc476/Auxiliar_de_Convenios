...
interface AddMunicipioModalProps {
  ...
  onSuccess: (newMunicipio: MunicipioDados) => void;
}
...
export default function AddMunicipioModal({
  open,
  onOpenChange,
  onSuccess,
}: AddMunicipioModalProps) {
  const [formData, setFormData] = useState<MunicipioDados>({
    municipio: "",
    cnpj: "",
    prefeito: "",
    endereco: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Chamar a API para criar o município
      const response = await apiService.createMunicipio(formData);
      
      // Obter o novo município com o ID retornado pelo backend
      const newMunicipio = {
        ...formData,
        id: response.id
      };
      ...
      toast({
        ...
        description: `O município ${formData.municipio} foi adicionado com sucesso!`,
        duration: 5000,
      });
      
      // Callback de sucesso
      onSuccess(newMunicipio);
      ...
      setError(error.message || "Ocorreu um erro ao salvar os dados.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-6 rounded-t-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-blue-100 mt-1">
              Cadastre informações de um novo município
            </DialogDescription>
          </DialogHeader>
        </div>
          ...
          <DialogFooter className="mt-8 pt-4 border-t border-gray-700 flex flex-col sm:flex-row gap-2">
            ...
            >
              {loading ? (
                ...
                <>
                  <Save className="mr-2 h-4 w-4" /> Adicionar Município
                </>
              )}
            </Button>
          </DialogFooter>
